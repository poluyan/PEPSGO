...
//		{
//			TFloat range_normalized = (ub - lb)/TFloat(gridn);
//			size_t it = get_lb<size_t, core::Real>(lb, ub, gridn, value);
//			if(it >= gridn)
//				return gridn - 1;
//			else if(it == 0)
//				return 0;
...
//			size_t ngt = get_the_closest_grid_node_to_the_value<size_t, core::Real>(-numeric::NumericTraits<core::Real>::pi(), numeric::NumericTraits<core::Real>::pi(), phipsi_grid_number[index], radian);
//			size_t t = dist > 0 ? dist - 1 : 0;
//			if(t != ngt)
//			{
//				for(size_t i = 0; i != phipsi_grids_radians[index].size() - 1; i++)
//				{
//					std::cout << phipsi_grids_radians[index][i] + phipsi_grids_dx[index] << '\t' << -numeric::NumericTraits<core::Real>::pi() + i*(2.0*numeric::NumericTraits<core::Real>::pi())/core::Real(phipsi_grids_radians[index].size() - 1) + 2.0*numeric::NumericTraits<core::Real>::pi()/(2.0*(phipsi_grids_radians[index].size()-1)) << std::endl;
//				}
//				std::cout << "wow!! " << '\t' << radian << '\t' << std::abs(radian - (phipsi_grids_radians[index][t] + phipsi_grids_dx[index])) << " vs " <<
//				          std::abs(radian + numeric::NumericTraits<core::Real>::pi() - ngt*(2.0*numeric::NumericTraits<core::Real>::pi())/core::Real(phipsi_grid_number[index] - 1)) << std::endl;
//				std::cout << "wow!! " << '\t' << radian << '\t' << phipsi_grids_radians[index][t] + phipsi_grids_dx[index] << " vs " <<
//				          -numeric::NumericTraits<core::Real>::pi() + ngt*(2.0*numeric::NumericTraits<core::Real>::pi())/core::Real(phipsi_grid_number[index] - 1) << std::endl;
//				std::cin.get();
//			}
